...
from utils.sceneflow_util import pixel2pts_ms, pts2pixel_ms, reconstructImg, reconstructPts, projectSceneFlow2Flow, flow_warp
...
class Loss_SceneFlow_SelfSup(nn.Module):
    ...
    def depth_loss_left_img(self, disp_l, disp_r, img_l_aug, img_r_aug, ii):
        ...
        return loss_img + self._disp_smooth_w * loss_smooth, left_occ, img_r_warp

    def optFlow_loss(self,opt_flow_f, opt_flow_b, img_l1_aug, img_l2_aug, disp_occ_l1, disp_occ_l2, ii):
        occ_map_b = _adaptive_disocc_detection(opt_flow_f).detach() * disp_occ_l2
        occ_map_f = _adaptive_disocc_detection(opt_flow_b).detach() * disp_occ_l1
        img_l2_warp = flow_warp(img_l2_aug, opt_flow_f)
        img_l1_warp = flow_warp(img_l1_aug, opt_flow_b)
        ...
        loss_opFlow_s = ( (_smoothness_motion_2nd(opt_flow_f, img_l1_aug, beta=10.0)).mean() + (_smoothness_motion_2nd(opt_flow_b, img_l2_aug, beta=10.0)).mean() ) / (2 ** ii)

        opticalF_losses = loss_im + self._optFlow_smooth_w * loss_opFlow_s

        return opticalF_losses, loss_im, loss_opFlow_s, img_l2_warp, img_l1_warp
    ...
    def sceneflow_loss(self, sf_f, sf_b, disp_l1, disp_l2, disp_occ_l1, disp_occ_l2, k_l1_aug, k_l2_aug, img_l1_aug, img_l2_aug, aug_size, ii):
        ...
        flow_f, _ = projectSceneFlow2Flow(k1_scale, sf_f, disp_l1)
        flow_b, _ = projectSceneFlow2Flow(k2_scale, sf_b, disp_l2)
        ...
        # sf ---> 2d flow
        #img_l2_warp_flow_f = self._warping_layer(img_l2_aug, flow_f)
        #img_l1_warp_flow_b = self._warping_layer(img_l1_aug, flow_b)
        ...
        #sceneflow_loss = loss_im  + self._sf_3d_sm * loss_3d_s
        #loss_pts = 0.0
        
        return sceneflow_loss, loss_im, loss_pts, loss_3d_s, img_l1_warp, img_l2_warp
    ...
    def forward(self, output_dict, target_dict):
        ...
        n_prediction = len(output_dict['flow_f'])
        ...
        for ii in range(n_prediction):
            sf_f, sf_b, disp_l1, disp_l2, disp_r1, disp_r2 = output_dict['flow_f'][ii], output_dict['flow_b'][ii], output_dict['disp_l1'][ii], output_dict['disp_l2'][ii], disp_r1_dict[ii], disp_r2_dict[ii]
            
            i_weight = self._gamma**(n_prediction - ii - 1)
            ...
            ## Disp Loss
            loss_disp_l1, disp_occ_l1, img_r1_warp = self.depth_loss_left_img(disp_l1, disp_r1, img_l1_aug, img_r1_aug, ii)
            loss_disp_l2, disp_occ_l2, img_r2_warp = self.depth_loss_left_img(disp_l2, disp_r2, img_l2_aug, img_r2_aug, ii)
            loss_dp_sum = loss_dp_sum + (loss_disp_l1 + loss_disp_l2) * i_weight
            
            
            ## Sceneflow Loss           
            loss_sceneflow, loss_im, loss_pts, loss_3d_s, img_l1_warp, img_l2_warp = self.sceneflow_loss(sf_f, sf_b, 
                                                                            disp_l1, disp_l2,
            ...
            loss_sf_sum = loss_sf_sum + loss_sceneflow * i_weight      
            loss_sf_2d = loss_sf_2d + loss_im * i_weight            
            loss_sf_3d = loss_sf_3d + loss_pts * i_weight
            loss_sf_sm = loss_sf_sm + loss_3d_s * i_weight
            #loss_sf_f = loss_sf_f + loss_sf_f_im * i_weight

            img_d1_wrap.append(img_r1_warp)
            img_d2_wrap.append(img_r2_warp)
            img_sf1_wrap.append(img_l1_warp)
            img_sf2_wrap.append(img_l2_warp)
        ...
        loss_dict = {}
        recons_imgs  = {}
        recons_imgs["img_r1_wrap"] = img_d1_wrap[0]
        recons_imgs["img_r2_wrap"] = img_d2_wrap[0]
        recons_imgs["img_sf1_wrap"] = img_sf1_wrap[0]
        recons_imgs["img_sf2_wrap"] = img_sf2_wrap[0]
...
class Loss_SceneFlow_SemiSupFinetune(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        loss_dict = {}
        unsup_loss_dict, recons_imgs = self._unsup_loss(output_dict, target_dict)
        ...
        for ii, sf_f in enumerate(output_dict['flow_f_pp']):
            ...
            out_flow, _ = projectSceneFlow2Flow(target_dict['input_k_l1'], sf_f_up, disp_l1)
...
def eval_module_disp_depth(gt_disp, gt_disp_mask, output_disp, gt_depth, output_depth):
    ...
    loss_dict["error_map"] = d_valid_epe
...
def renderImgsSceneFlow(output_dict, target_dict, mode):
        import cv2
        ...
        img_l1 = interpolate2d_as(target_dict["input_l1"], sf_f)
        img_l2 = interpolate2d_as(target_dict["input_l2"], sf_b)
        ## scale
        local_scale = torch.zeros_like(aug_size)
        local_scale[:, 0] = h_dp
        local_scale[:, 1] = w_dp         
        ...
        ## Image reconstruction loss
        img_l2_warp = reconstructImg(coord1, img_l2).squeeze(0).cpu().numpy().transpose(1,2,0)
        img_l1_warp = reconstructImg(coord2, img_l1).squeeze(0).cpu().numpy().transpose(1,2,0)
        cv2.imwrite('./rendered_imgs/{}_img_sf_l2_warp.png'.format(mode), cv2.cvtColor(img_l2_warp*255, cv2.COLOR_RGB2BGR))
        cv2.imwrite('./rendered_imgs/{}_img_sf_l1_warp.png'.format(mode), cv2.cvtColor(img_l1_warp*255, cv2.COLOR_RGB2BGR))

def save_rendImgs(img_l1, img_l2, img_rend, mode):
    import cv2
    img_l1 = img_l1.squeeze(0).cpu().numpy()
    cv2.imwrite('./rendered_imgs/img_l1.png', cv2.cvtColor(img_l1.transpose(1,2,0)*255, cv2.COLOR_RGB2BGR))
    img_l2 = img_l2.squeeze(0).cpu().numpy()
    cv2.imwrite('./rendered_imgs/img_l2.png', cv2.cvtColor(img_l2.transpose(1,2,0)*255, cv2.COLOR_RGB2BGR))
    rend_l = img_rend.squeeze(0).cpu().numpy()
    rend_l = rend_l.transpose(1,2,0)
    cv2.imwrite('./rendered_imgs/rend_l_{}.png'.format(mode), cv2.cvtColor(rend_l*255, cv2.COLOR_RGB2BGR))
    
def renderImageDisp(output_dict, target_dict, mode):
    gt_disp = target_dict['target_disp']
    img_l1_aug = interpolate2d_as(target_dict["input_l1"], gt_disp)
    img_l2_aug = interpolate2d_as(target_dict["input_l2"], gt_disp)
    img_r1_aug = interpolate2d_as(target_dict["input_r1"], gt_disp)
    img_r2_aug = interpolate2d_as(target_dict["input_r2"], gt_disp)
    
    # save rendered image
    img_l_recons_gt = _generate_image_left(img_r1_aug, gt_disp)
    save_rendImgs(img_l1_aug, img_l2_aug, img_l_recons_gt, mode + '_gt')
    disp_l1 =  interpolate2d_as(output_dict["disp_l1"][0], gt_disp)
    img_l_recons_est = _generate_image_left(img_r1_aug, disp_l1)
    save_rendImgs(img_l1_aug, img_l2_aug, img_l_recons_est, mode)
...
class Eval_SceneFlow_KITTI_Train(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        #renderImgsSceneFlow(output_dict, target_dict, mode='msf-rp')
        #renderImageDisp(output_dict, target_dict, mode='msf-rp')
        loss_dict = {}
        ...
        out_disp_l1 = interpolate2d_as(output_dict["disp_l1_pp"][-1], gt_disp, mode="bilinear") * width
        out_depth_l1 = _disp2depth_kitti_K(out_disp_l1, intrinsics[:, 0, 0])
        out_depth_l1 = torch.clamp(out_depth_l1, 1e-3, 80)
        ...
        ## 2D Flow
        #out_op_flow = interpolate2d_as(output_dict['op_flow_f_pp'][-1], gt_flow, mode='bilinear')
        #print(gt_flow.min(), gt_flow.max(), 'gt flow')
        #print(out_op_flow.min(), out_op_flow.max(), 'of')
        #valid_epe_opf = _elementwise_epe(out_op_flow, gt_flow) * gt_flow_mask
        #loss_dict["of_epe"] = (valid_epe_opf.view(batch_size, -1).sum(1)).mean() / 91875.68
        #op_flow_outlier_epe = (valid_epe_opf > 3).float() * ((valid_epe_opf / flow_gt_mag) > 0.05).float() * gt_flow_mask
        #loss_dict["of_f1"] = (op_flow_outlier_epe.view(batch_size, -1).sum()).mean() / 91875.68
        
        # 3D Flow
        out_sceneflow = interpolate2d_as(output_dict['flow_f_pp'][-1], gt_flow, mode="bilinear")
        out_flow, _ = projectSceneFlow2Flow(target_dict['input_k_l1'], out_sceneflow, output_dict["out_disp_l_pp"])
        ## Flow Eval
        #print(out_flow.min(), out_flow.max(), 'sf2of')
...
class Eval_Disp_Only(nn.Module):
    ...
    def forward(self, output_dict, target_dict):
        ...
        out_disp_l1 = interpolate2d_as(output_dict["disp_l1_pp"][-1], gt_disp, mode="bilinear") * gt_disp.size(3)
        out_depth_l1 = _disp2depth_kitti_K(out_disp_l1, intrinsics[:, 0, 0])
        out_depth_l1 = torch.clamp(out_depth_l1, 1e-3, 80)
