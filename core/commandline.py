...
def _parse_arguments():
    ...
    add("--tbIter", type=int, default=10)
    add("--correlation_cuda_enabled", default=True)
    add("--small", default=False)
    add("--iters", type=int, default=6)
    add("--mixed_precision", type=tools.str2bool, default=False)
    add("--gamma", type=float, default=0.8)
    add("--world_size", default=1, type=int, help='number of distributed processes')
    add("--dist_url", default='env://', help='url used to set up distributed training')
    add('--device', default='cuda', help='device to use for training / testing')
